...
ncclResult_t ncclInitKernelsForDevice(int cudaArch, int maxSharedMem, size_t* maxStackSize) {
  ...
  for (int k = 0; k < KernelCount; k++) {
    void* fn = ncclKerns[k].kernelFn;
      ...
      hipError_t errcode = hipFuncGetAttributes(&attr, reinterpret_cast<const void*>(fn));
      if (errcode != hipSuccess) continue; // Silently ignore failures
      if (maxStackSize) {
        if (attr.localSizeBytes > *maxStackSize) *maxStackSize = attr.localSizeBytes;
      }
      if (carveout) {
        CUDACHECKGOTO(hipFuncSetAttribute(reinterpret_cast<const void*>(fn),
      ...
      if (ncclMaxSharedMem != 0) {
        ...
        CUDACHECKGOTO(hipFuncSetAttribute(reinterpret_cast<const void*>(fn),
          hipFuncAttributeMaxDynamicSharedMemorySize, sharedMemSize),
          result, next_kernel);
      }
    next_kernel:;
    }
...
namespace {
  ...
  ncclResult_t uploadWork_cleanup_fn(
    ...
    free(me->hostBuf);
    CUDACHECK(hipEventDestroy(me->base.event));
    free(me);
    return ncclSuccess;
  }
}
...
static ncclResult_t uploadWork(struct ncclComm* comm, struct ncclKernelPlan* plan) {
  ...
  case ncclDevWorkStorageTypePersistent:
    { ncclResult_t result = ncclSuccess;
      ...
      hipStream_t deviceStream;

      CUDACHECKGOTO(hipThreadExchangeStreamCaptureMode(&mode), result, fail);
      ...
      CUDACHECKGOTO(hipMallocAsync(&fifoBufDev, workBytes, comm->memPool, deviceStream), result, fail);
      plan->workBufPersistent = fifoBufDev;
      plan->kernelArgs->workBuf = fifoBufDev;

      // coverity[uninit_use_in_call:FALSE] => fifoBufHost is never NULL
      CUDACHECKGOTO(hipMemcpyAsync(fifoBufDev, fifoBufHost, workBytes, hipMemcpyDefault, deviceStream), result, fail);
      hipEvent_t memcpyDone;
      CUDACHECKGOTO(hipEventCreateWithFlags(&memcpyDone, hipEventDisableTiming), result, fail);
      CUDACHECKGOTO(hipEventRecord(memcpyDone, deviceStream), result, fail);
    ...
    finish_scope:
      if (mode != hipStreamCaptureModeRelaxed) (void)hipThreadExchangeStreamCaptureMode(&mode);
      return result;
    fail:
...
static ncclResult_t reclaimPlan(struct ncclComm* comm, struct ncclCommCallback* me) {
  ...
  if (plan->persistent) {
    ...
    if (plan->workStorageType == ncclDevWorkStorageTypePersistent) {
      ...
      CUDACHECK(hipFree(plan->workBufPersistent));
      CUDACHECK(hipThreadExchangeStreamCaptureMode(&mode));
    }
  }
  // Free coll tasks
...
ncclResult_t ncclLaunchPrepare(struct ncclComm* comm) {
  ...
  if (planner->nTasksColl + planner->nTasksP2p != 0) {
    ...
    hipStream_t launchStream = planner->streams->stream;
    hipStream_t deviceStream, launchOrder;
    ...
    if (persistent || planner->numStreams != 1) {
      ...
      for (struct ncclCudaStreamList* l=planner->streams->next; l != nullptr; l = l->next) {
        CUDACHECKGOTO(hipEventRecord(comm->sharedRes->scratchEvent, l->stream), result, failure);
        CUDACHECKGOTO(hipStreamWaitEvent(launchStream, comm->sharedRes->scratchEvent, 0), result, failure);
      }
    ...
    if (!persistent && comm->sharedRes->persistentRefs) status = hipEventQuery(comm->sharedRes->hostStream.serialEvent);
    if (persistent || ncclCudaLaunchBlocking || status == hipErrorNotReady) {
      // We have to launch host tasks to push proxy args. We are careful to only
      // do this if necessary since host tasks impose a high performance cost in CUDA.
      bool acquired = false;
      hipStream_t hostStream;
      for (struct ncclKernelPlan* plan=planHead; plan != nullptr; plan = plan->next) {
        if (plan->hasProxyOps) {
          ...
          CUDACHECKGOTO(hipLaunchHostFunc(hostStream, hostStreamPlanCallback, plan), result, failure);
        }
      }
      if (acquired) {
...
ncclResult_t ncclLaunchKernel(struct ncclComm* comm, struct ncclKernelPlan* plan) {
  ...
  // hipFunction_t fn;
  // CUDACHECK(hipGetFuncBySymbol(&fn, sym));
  ...
  hipFunction_t fn;
  CUDACHECKGOTO(hipGetFuncBySymbol(&fn, sym), ret, do_return);
  ...
  if (CUDART_VERSION >= 11080 && driverVersion >= 11080) {
    ...
    latency_profiler::collTraceRecordStartEvent(comm, launchStream, event.get());
    CUCHECKGOTO(hipModuleLaunchKernel(fn, grid.x, grid.y, grid.z, block.x, block.y, block.z, smem, launchStream, nullptr, extra), ret, do_return);
    latency_profiler::collTraceRecordEndEvent(comm, plan, launchStream, std::move(event));
  }
#endif
  // Standard kernel launch
  //hipModuleLaunchKernel(sym, grid.x, grid.y, grid.z, block.x, block.y, block.z, smem, launchStream, nullptr, extra);
  latency_profiler::collTraceRecordStartEvent(comm, launchStream, event.get());
  CUDACHECKGOTO(hipLaunchKernel(reinterpret_cast<const void*>(sym), grid, block, extra, smem, launchStream), ret, do_return);
  latency_profiler::collTraceRecordEndEvent(comm, plan, launchStream, std::move(event));
...
ncclResult_t ncclLaunchFinish(struct ncclComm* comm) {
  ...
  if (!ncclIntruQueueEmpty(&planner->planQueue)) {
    ...
    bool capturing = ncclCudaGraphValid(planner->capturingGraph);
    //hipStream_t launchStream = planner->streams->stream; // First user stream gets launch // unused variable - compiler warning
    hipStream_t deviceStream, launchOrder;

    hipEvent_t finishedEvent = comm->sharedRes->scratchEvent;
    ...
    if (comm->workFifoProduced - comm->workFifoProducedLastRecorded > comm->workFifoBytes/8) {
      ...
      CUDACHECK(hipEventCreateWithFlags(&comm->sharedRes->scratchEvent, hipEventDisableTiming));
    }

    if (capturing || planner->numStreams != 1) {
      // CUDACHECK(hipEventRecord(finishedEvent, launchStream));
      ...
      // So we could do this:
      //   CUDACHECK(hipStreamWaitEvent(deviceStream, finishedEvent, 0));
      // But instead we do:
      ...
      for (struct ncclCudaStreamList* l=planner->streams->next; l != nullptr; l = l->next) {
        CUDACHECK(hipStreamWaitEvent(l->stream, finishedEvent, 0));
      }
    }
    enum ncclImplicitOrder implicitOrder;
    ...
    if (implicitOrder != ncclImplicitOrderNone) {
      ...
      CUDACHECK(hipStreamWaitEvent(launchOrder, implicitOrder == ncclImplicitOrderLaunch ? comm->sharedRes->launchEvent : finishedEvent));
