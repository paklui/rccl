...
struct ncclSharedResources {
  ...
  int persistentRefs;
  hipEvent_t launchEvent, scratchEvent;
...
struct ncclComm {
  ...
  * asynchronous NCCL operations. */
  ...
  // Device side of the communicator (for hipFree's)
  ...
  size_t baseStride;
  size_t symAllocHead;
  hipMemGenericAllocationHandle_t symMCHandle;
...
inline ncclResult_t ncclCommPollEventCallbacks(struct ncclComm *comm, bool waitSome) {
  ...
  while (true) {
    ...
    hipError_t ok;
    if (waitSome) {
      ok = hipEventSynchronize(cb->event);
      waitSome = false;
    } else {
      ok = hipEventQuery(cb->event);
      if (ok == hipErrorNotReady) break;
    }
