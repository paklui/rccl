...
template<>
struct Apply_Cast<__hip_bfloat16, float, /*EltPerPack=*/2> {
  __device__ __forceinline__ static BytePack<4*2> cast(BytePack<2*2> a) {
    return toPack(__bfloat1622float2(fromPack<__hip_bfloat162>(a)));
  }
};
template<>
struct Apply_Cast<float ,__hip_bfloat16, /*EltPerPack=*/2> {
  __device__ __forceinline__ static BytePack<2*2> cast(BytePack<4*2> a) {
...
EASY_CAST(__hip_fp8_e5m2_fnuz, float, 2, __hip_fp8x2_e5m2_fnuz, float2)
EASY_CAST(__hip_fp8_e5m2_fnuz, float, 4, __hip_fp8x4_e5m2_fnuz, float4)

EASY_CAST(__hip_fp8_e4m3_fnuz, float, 2, __hip_fp8x2_e4m3_fnuz, float2)
EASY_CAST(__hip_fp8_e4m3_fnuz, float, 4, __hip_fp8x4_e4m3_fnuz, float4)
#endif
#undef EASY_CAST
...
SPECIALIZE_REDUCE(FuncMinMax, half, 1, half, fn.isMinNotMax ? __hmin(x, y) : __hmax(x, y))
  ...
  SPECIALIZE_REDUCE(FuncSum, __hip_bfloat16, 1, __hip_bfloat16, __hadd(x, y))
  // coverity[copy_constructor_call]
  SPECIALIZE_REDUCE(FuncSum, __hip_bfloat16, 2, __hip_bfloat162, __hadd2(x, y))
  SPECIALIZE_REDUCE(FuncProd, __hip_bfloat16, 1, __hip_bfloat16, __hmul(x, y))
  // coverity[copy_constructor_call]
  SPECIALIZE_REDUCE(FuncProd, __hip_bfloat16, 2, __hip_bfloat162, __hmul2(x, y))
  SPECIALIZE_REDUCE(FuncMinMax, __hip_bfloat16, 1, __hip_bfloat16, fn.isMinNotMax ? __hmin(x, y) : __hmax(x, y))
  // coverity[copy_constructor_call]
  SPECIALIZE_REDUCE(FuncMinMax, __hip_bfloat16, 2, __hip_bfloat162, fn.isMinNotMax ? __hmin2(x, y) : __hmax2(x, y))
#else
  ...
  SPECIALIZE_REDUCE(FuncSum, __hip_fp8_e4m3_fnuz, 1, __hip_fp8_e4m3_fnuz, __hip_fp8_e4m3_fnuz(__hadd(__half(x),__half(y))))
  SPECIALIZE_REDUCE(FuncSum, __hip_fp8_e4m3_fnuz, 2, __hip_fp8x2_e4m3_fnuz, __hip_fp8x2_e4m3_fnuz(__hadd2(__half2(x),__half2(y))))
  SPECIALIZE_REDUCE(FuncProd, __hip_fp8_e4m3_fnuz, 1, __hip_fp8_e4m3_fnuz, __hip_fp8_e4m3_fnuz(__hmul(__half(x),__half(y))))
  SPECIALIZE_REDUCE(FuncProd, __hip_fp8_e4m3_fnuz, 2, __hip_fp8x2_e4m3_fnuz, __hip_fp8x2_e4m3_fnuz(__hmul2(__half2(x),__half2(y))))
  SPECIALIZE_REDUCE(FuncMinMax, __hip_fp8_e4m3_fnuz, 1, __hip_fp8_e4m3_fnuz, __hip_fp8_e4m3_fnuz(fn.isMinNotMax ? __hmin(__half(x),__half(y)) : __hmax(__half(x),__half(y))))
  SPECIALIZE_REDUCE(FuncMinMax, __hip_fp8_e4m3_fnuz, 2, __hip_fp8x2_e4m3_fnuz, __hip_fp8x2_e4m3_fnuz(fn.isMinNotMax ? __hmin2(__half2(x),__half2(y)) : __hmax2(__half2(x),__half2(y))))

  SPECIALIZE_REDUCE(FuncSum, __hip_fp8_e5m2_fnuz, 1, __hip_fp8_e5m2_fnuz, __hip_fp8_e5m2_fnuz(__hadd(__half(x),__half(y))))
  SPECIALIZE_REDUCE(FuncSum, __hip_fp8_e5m2_fnuz, 2, __hip_fp8x2_e5m2_fnuz, __hip_fp8x2_e5m2_fnuz(__hadd2(__half2(x),__half2(y))))
  SPECIALIZE_REDUCE(FuncProd, __hip_fp8_e5m2_fnuz, 1, __hip_fp8_e5m2_fnuz, __hip_fp8_e5m2_fnuz(__hmul(__half(x),__half(y))))
  SPECIALIZE_REDUCE(FuncProd, __hip_fp8_e5m2_fnuz, 2, __hip_fp8x2_e5m2_fnuz, __hip_fp8x2_e5m2_fnuz(__hmul2(__half2(x),__half2(y))))
  SPECIALIZE_REDUCE(FuncMinMax, __hip_fp8_e5m2_fnuz, 1, __hip_fp8_e5m2_fnuz, __hip_fp8_e5m2_fnuz(fn.isMinNotMax ? __hmin(__half(x), __half(y)) : __hmax(__half(x), __half(y))))
  SPECIALIZE_REDUCE(FuncMinMax, __hip_fp8_e5m2_fnuz, 2, __hip_fp8x2_e5m2_fnuz, __hip_fp8x2_e5m2_fnuz(fn.isMinNotMax ? __hmin2(__half2(x), __half2(y)) : __hmax2(__half2(x), __half2(y))))
#else
...
struct FuncPreMulSum<half> {
  ...
  template<>
  // Coverity recommends the users of this type to use std::move in certain cases but,
  // given that __hip_bfloat16 is a scalar, a plain copy will be just as efficient.
  // coverity[moveable_type]
  struct FuncPreMulSum<hip_bfloat16> {
    using EltType = hip_bfloat16;
  #if __CUDA_ARCH__ >= 800
    __hip_bfloat162 scalar;
    __device__ __forceinline__ FuncPreMulSum(uint64_t opArg=0) {
      union { uint64_t u64; __hip_bfloat16 val; };
      u64 = opArg;
      scalar.x = val;
      scalar.y = val;
    }
...
#if __CUDA_ARCH__ >= 900
  template<>
  struct FuncPreMulSum<__hip_fp8_e4m3_fnuz> {
    using EltType = __hip_fp8_e4m3_fnuz;
    __half2 scalar2;
    __device__ __forceinline__ FuncPreMulSum(uint64_t opArg) {
      union { uint64_t u64; __hip_fp8_storage_t val; };
      u64 = opArg;
      scalar2.x = __half(__hip_cvt_fp8_to_halfraw(val, __HIP_E4M3_FNUZ));
      scalar2.y = scalar2.x;
    }
  };

  template<>
  struct FuncPreMulSum<__hip_fp8_e5m2_fnuz> {
    using EltType = __hip_fp8_e5m2_fnuz;
    __half2 scalar2;
    __device__ __forceinline__ FuncPreMulSum(uint64_t opArg) {
      union { uint64_t u64; __hip_fp8_storage_t val; };
      u64 = opArg;
      scalar2.x = __half(__hip_cvt_fp8_to_halfraw(val, __HIP_E5M2_FNUZ));
      scalar2.y = scalar2.x;
    }
  };
#else
...
// Apply_PreOp of FuncPreMulSum for bfloat16.
  ...
  struct Apply_PreOp<FuncPreMulSum<hip_bfloat16>, /*EltPerPack=*/1> {
    ...
    __device__ __forceinline__ static BytePack<sizeof(hip_bfloat16)> preOp(
      ...
      #if __CUDA_ARCH__ >= 800
        return toPack<__hip_bfloat16>(__hmul(fromPack<__hip_bfloat16>(a), fn.scalar.x));
      #else
    ...
    struct Apply_PreOp<FuncPreMulSum<hip_bfloat16>, /*EltPerPack=*/2> {
      static constexpr bool IsIdentity = false;
      __device__ __forceinline__ static BytePack<sizeof(__hip_bfloat162)> preOp(
          FuncPreMulSum<__hip_bfloat16> fn, BytePack<sizeof(__hip_bfloat162)> a
        ) {
        return toPack<__hip_bfloat162>(__hmul2(fromPack<__hip_bfloat162>(a), fn.scalar));
      }
    };
  #endif
#endif
...
#if __CUDA_ARCH__ >= 900
  template<>
  struct Apply_PreOp<FuncPreMulSum<__hip_fp8_e4m3_fnuz>, /*EltPerPack=*/1> {
    static constexpr bool IsIdentity = false;
    __device__ __forceinline__ static BytePack<sizeof(__hip_fp8_e4m3_fnuz)> preOp(
        FuncPreMulSum<__hip_fp8_e4m3_fnuz> fn, BytePack<sizeof(__hip_fp8_e4m3_fnuz)> a
      ) {
      return toPack<__hip_fp8_e4m3_fnuz>(__hip_fp8_e4m3_fnuz(__hmul(__half(fromPack<__hip_fp8_e4m3_fnuz>(a)), fn.scalar2.x)));
    }
  };
  template<>
  struct Apply_PreOp<FuncPreMulSum<__hip_fp8_e4m3_fnuz>, /*EltPerPack=*/2> {
    static constexpr bool IsIdentity = false;
    __device__ __forceinline__ static BytePack<sizeof(__hip_fp8x2_e4m3_fnuz)> preOp(
        FuncPreMulSum<__hip_fp8_e4m3_fnuz> fn, BytePack<sizeof(__hip_fp8x2_e4m3_fnuz)> a
      ) {
      return toPack<__hip_fp8x2_e4m3_fnuz>(__hip_fp8x2_e4m3_fnuz(__hmul2(__half2(fromPack<__hip_fp8x2_e4m3_fnuz>(a)), fn.scalar2)));
    }
  };

  template<>
  struct Apply_PreOp<FuncPreMulSum<__hip_fp8_e5m2_fnuz>, /*EltPerPack=*/1> {
    static constexpr bool IsIdentity = false;
    __device__ __forceinline__ static BytePack<sizeof(__hip_fp8_e5m2_fnuz)> preOp(
        FuncPreMulSum<__hip_fp8_e5m2_fnuz> fn, BytePack<sizeof(__hip_fp8_e5m2_fnuz)> a
      ) {
      return toPack<__hip_fp8_e5m2_fnuz>(__hip_fp8_e5m2_fnuz(__hmul(__half(fromPack<__hip_fp8_e5m2_fnuz>(a)), fn.scalar2.x)));
    }
  };
  template<>
  struct Apply_PreOp<FuncPreMulSum<__hip_fp8_e5m2_fnuz>, /*EltPerPack=*/2> {
    static constexpr bool IsIdentity = false;
    __device__ __forceinline__ static BytePack<sizeof(__hip_fp8x2_e5m2_fnuz)> preOp(
        FuncPreMulSum<__hip_fp8_e5m2_fnuz> fn, BytePack<sizeof(__hip_fp8x2_e5m2_fnuz)> a
      ) {
      return toPack<__hip_fp8x2_e5m2_fnuz>(__hip_fp8x2_e5m2_fnuz(__hmul2(__half2(fromPack<__hip_fp8x2_e5m2_fnuz>(a)), fn.scalar2)));
    }
  };
#else
  template<>
...
struct Apply_LoadMultimem {
  ...
  DEFINE_Apply_LoadMultimem_minmax_v4_and_xparts(half, f16x2, 4)
    ...
    DEFINE_Apply_LoadMultimem_minmax_v4_and_xparts(hip_bfloat16, bf16x2, 4)
      ...
      DEFINE_Apply_LoadMultimem_sum_v4_and_xparts(__hip_fp8_e4m3_fnuz, e4m3x4, 4)
      DEFINE_Apply_LoadMultimem_minmax_v4_and_xparts(__hip_fp8_e4m3_fnuz, e4m3x4, 4)
      DEFINE_Apply_LoadMultimem_sum_v4_and_xparts(__hip_fp8_e5m2_fnuz, e5m2x4, 4)
      DEFINE_Apply_LoadMultimem_minmax_v4_and_xparts(__hip_fp8_e5m2_fnuz, e5m2x4, 4)
    #else
